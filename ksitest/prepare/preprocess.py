...
def preprocess_snp(snp_data_path):
    ...
    df = snp_data.pivot_table(index='animal_id', columns='SNP Name', values='Allele1 - AB', aggfunc='first')
    threshold = len(df) * 0.2
    df_cleaned = Parallel(n_jobs=-1)(
        delayed(clean_snp_column)(df[col]) for col in tqdm(df.columns, desc="Cleaning SNP columns")
    )
...
def preprocess_str(str_data_path):
    ...
    df = str_data.pivot_table(index='animal_id', columns='STR Name', values=['Allele1', 'Allele2'], aggfunc='first')
